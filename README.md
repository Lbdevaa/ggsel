...
**В процессе теста.** `npm test` поднимает API с БД в памяти и заглушки поставщиков A и B на случайных портах и прогоняет те же сценарии плюс сбои поставщиков (таймаут, 5xx, пустой пул, недоступный хост, fallback, повторная выдача). Около 2 секунд, 26 тестов.
