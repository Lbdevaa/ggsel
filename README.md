...
Скрипт [scripts/race.mjs](scripts/race.mjs) создаёт свои заказы, бьёт по API параллельными запросами и печатает по строке на сценарий. В конце проверяется инвариант: у поставщика списано ровно столько ключей, сколько заказов доведено до `delivered`. Сценарии описаны в [docs/races.md](docs/races.md).

**В процессе теста.** `npm test` поднимает API с БД в памяти и заглушку поставщика на случайных портах и прогоняет те же сценарии плюс сбои поставщика (таймаут, 5xx, пустой пул, недоступный хост). Около 1 секунды, 20 тестов.
